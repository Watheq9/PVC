...
def draw_line_graph(data, save_file, x_axis_label=None, y_axis_label=None, title=None,
    ...
    # Font settings - couldn't get it working yet
    #plt.rcParams.update({
...
def draw_stacked_column_chart(categories, models, percentages, save_file, x_axis_label=None, y_axis_label=None, title=None,
    ...
    # Add a grid on top of the ticks for improved readibility
    # plt.grid(axis='x', color='0.9', linestyle='--')
...
def draw_scatter_plot(x, y, x_axis_label=None, y_axis_label=None, title=None,
                    pdf_x_dim = 8, pdf_y_dim = 6, fontsize = 10, y_min_value=0, y_max_value=1, 
                    x_min_value=0, x_max_value=1, color='blue', save_file=None, 
                    legend_label='', legeng_location='upper left'):
    # Set font size
    params = {'axes.labelsize': fontsize,'axes.titlesize': fontsize, 
              'legend.fontsize': fontsize, 'xtick.labelsize': fontsize - 4, 'ytick.labelsize': fontsize - 4}
    plt.rcParams.update(params)
    # Subplots can be used to put multiple plots in one frame
    fig, ax = plt.subplots(figsize=(pdf_x_dim, pdf_y_dim))
    ...
    plt.title(title)
    if legend_label != "":
        plt.legend(labels=[legend_label], loc=legeng_location)
